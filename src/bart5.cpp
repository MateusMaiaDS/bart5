...
// Updating Z
void update_z(arma::vec &z_vec,
              arma::vec &y_hat,
              modelParam &data){

        for(int i = 0; i < data.x_train.n_rows; i++){
                if(data.y(i)==1){
                        z_vec(i) = up_tn_sampler(y_hat(i),0.0);
                } else {
                        z_vec(i) = lw_tn_sampler(y_hat(i),0.0);
                }
        }
}
...
Rcpp::List cppbart_CLASS(arma::mat x_train,
        ...
        // Initialising the z_vector
        arma::vec z_train(data.x_train.n_rows,arma::fill::zeros);
        ...
        for(int i = 0;i<data.n_mcmc;i++){
                ...
                for(int t = 0; t<data.n_tree;t++){
                        ...
                        // Updating the partial residuals
                        if(data.n_tree>1){
                                partial_residuals = z_train-sum_exclude_col(tree_fits_store,t);

                        } else {
                                partial_residuals = z_train;
                        }
